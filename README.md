...
Dimensional compression: Multidimensional political space collapses onto dominant axes
Geometric sorting: Individuals migrate toward cluster centers (VDR⁻¹ peaks)
Dynamical tightening: Party discipline increases χ (even as VDR⁻¹ stabilizes)
Metastable fragmentation: Intra-party factions emerge (both χ and VDR⁻¹ decline)
...
Python Dependencies
bashpip install numpy pandas matplotlib scipy scikit-learn networkx
Required packages:
...
bashpython congress_full_final.py
Outputs:
...
Era 1 (1st–63rd, 1789–1915): High volatility, episodic realignments
Era 2 (63rd–71st, 1913–1931): χ minimum (Republican dominance, internal heterogeneity)
Era 3 (71st–85th, 1929–1957): χ recovery, New Deal initiates 91-year sorting
Era 4 (85th–96th, 1957–1979): Civil Rights fragmentation
Era 5 (96th–114th, 1979–2017): Modern polarization, geometric-dynamical decoupling
Era 6 (115th–119th, 2017–present): Emergent intra-party factionalism


Modern Era (1967–2024): Four Regimes
bashpython congress_modern_final.py
Outputs:
...
Pluralistic (90th–103rd, 1967–1993): χ oscillates, center density elevated (mean 0.46)
Sharp transition (103rd–104th, 1993–1997): Gingrich Revolution, center drops (0.37→0.30)
Deep polarization (104th–115th, 1997–2017): χ rises to 0.83, VDR⁻¹ declines (decoupling)
Emergent factionalism (115th–119th, 2017–present): Both χ and VDR⁻¹ decline simultaneously


2. Kuramoto-Homophily Simulations
1D Simulations
bashpython sim_1d.py
Outputs:
...
Civil War maximum: VDR⁻¹ = 0.939 at 38th Congress (1863), highest in 235 years
New Deal inflection: 73rd Congress (1933) begins 91-year center density decline
Geometric sorting peak: 96th Congress (1979) shows VDR⁻¹ = 0.40
Dynamical maximum: 112th Congress (2010, Tea Party) shows χ = 0.83
Decoupling confirmed: VDR⁻¹ peaks 31 years before χ peaks

Theoretical (Simulations)

VDR⁻¹ is robust early-warning signal: Leads R(t) by ~100 time units (1D), ~40 samples (2D)
Multistability is universal: Identical parameters yield distinct regimes across seeds
Dimensional frustration: 2D reduces final R by ~20% vs. 1D (geometric coordination cost)
Distribution moments: Kurtosis ≈ -1.45 is dimension-independent (universal attractor statistics)


Data Sources
Congressional Voting Records

Source: Voteview DW-NOMINATE scores (https://voteview.com/)
Citation: Lewis, Jeffrey B., Keith Poole, Howard Rosenthal, Adam Boche, Aaron Rudkin, and Luke Sonnet (2023). Voteview: Congressional Roll-Call Votes Database. https://voteview.com/
...
License
Data is used under Voteview's CC BY 4.0 license. See https://voteview.com/about for details.

Replication Instructions
Full Replication (all analyses)
bash# Congressional analysis
...
Each individual i is represented by a d-dimensional state vector x_i(t) ∈ ℝ^d encoding political attitudes. The dynamics are:
ẋi = bi + ∑j gij F(xj − xi) + η_i(t)
where:

b_i: Intrinsic drift (personality, exogenous shocks)
G = (g_ij): Weighted influence matrix
...
χ(t): Empirical proxy via dominant eigenvalue of cosine similarity matrix
VDR⁻¹(t): Within-cluster variance / between-cluster separation (via GMM with BIC)
Center density: Fraction within ±0.3 on first ideological dimension
...
Data attribution: Congressional voting data from Voteview (CC BY 4.0). See Data Sources above.
